...
namespace reactamorty_api.Controllers
{
    ...
    public class CharactersController : ControllerBase
    {
        ...
        {
            var characterData =
                new CharacterData(page == 0 ? 1 : page, name ?? "", status ?? "", species ?? "", type ?? "",
                    gender ?? "");
            ...
            var paginatedCharacters = characters.GetRange(20 * (characterData.Page - 1),
                Math.Abs(characters.Count - (20 * (characterData.Page - 1))) >= 20
                    ? 20
            ...
            var charactersDto = _mapper.Map<List<Character>, CharactersDto>(paginatedCharacters, opt =>
            {
                opt.AfterMap((src, dest) =>
                {
                    ...
                    dest.Info.Next =
                        characterData.Page + 1 > dest.Info.Pages ? null : _characterService.FormatCharacterUrl(characterData, -1);
                    dest.Info.Prev =
                        characterData.Page - 1 < 1 ? null : _characterService.FormatCharacterUrl(characterData, 1);
                });
            });
